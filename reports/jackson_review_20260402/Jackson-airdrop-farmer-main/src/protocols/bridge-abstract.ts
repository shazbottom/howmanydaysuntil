...
/** Bridge ETH from Ethereum mainnet → Abstract via Relay */
export async function bridgeToAbstract(
  ...
  log.info(`Bridging ${formatEth(amount)} ETH → Abstract via Relay${to ? ` (to ${to.slice(0, 10)}...)` : ""}...`);
  ...
  // Step 2: Execute the L1 transaction from the quote
  let lastTxHash = "";
  ...
  for (const step of quote.steps) {
    ...
    for (const item of step.items) {
      ...
      log.info(`Sending Relay bridge tx to ${txData.to.slice(0, 12)}... value: ${formatEth(BigInt(txData.value))} ETH`);
      ...
      log.tx(lastTxHash, `Relay bridge ETH → Abstract`);
    }
  }
